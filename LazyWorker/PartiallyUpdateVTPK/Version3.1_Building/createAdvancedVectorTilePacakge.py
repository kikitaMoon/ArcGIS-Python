...
# What's New in 3.1?
# Add friendly error message to avoid unknown error.
# Limit the coordinate system to Geographic CS to avoid error. PCS is not supported now.
...
# Create Vector Tile Package Scheme for all kinds of Geographic Coordinate System
...
def modifyTilingSchemeFile(tileScheme):
    try:
        ...
        arcpy.AddError("Tile scheme path or folder name does not exist.")
...
def add_to_zip(original_zip, newfolder):
    try:
        ...
        arcpy.AddError("Original vtpk path or folder name does not exist.")
...
def execute(in_map,outVtpk,isOnline,vertex_count):
    try:
        ...
        if os.path.exists(originalVTPK):
        ...
        delete_zip_folder(vtpkDir)
        arcpy.AddError("Faild to Create Advanced Vector Tile Package.")
...
def main(argv=None):
    ...
    # arcpy.AddMessage("is Online:"+str(isOnline))
